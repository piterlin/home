...
var m_isUserInteracting = false,
  m_onMouseDownMouseX = 0,
  m_onMouseDownMouseY = 0,
  m_lon = 90,
  m_onMouseDownLon = 0,
  m_lat = 0,
  m_onMouseDownLat = 0,
...
function onDocumentMouseMoveRay(event) {
  m_rayMouse.x = ((event.clientX - m_container.offset().left) / m_container.width()) *
    2 - 1;
  m_rayMouse.y = -((event.clientY - m_container.offset().top) / m_container.height()) *
    2 + 1;
  ...
  if (intersects.length > 0) {
    ...
    if (m_INTERSECTED != intersects[0].object) {
      m_INTERSECTED.scale.x = m_INTERSECTED.scale.y = m_INTERSECTED.scale.z =
        0.05;
    } else {
      m_INTERSECTED = intersects[0].object;
      m_INTERSECTED.scale.x = m_INTERSECTED.scale.y = m_INTERSECTED.scale.z =
...
function onDocumentTouchStartRay(event) {
  if (event.touches.length == 1) {
    ...
    m_rayMouse.x = ((m_onPointerDownPointerX - m_container.offset().left) /
      m_container.width()) * 2 - 1;
    m_rayMouse.y = -((m_onPointerDownPointerY - m_container.offset().top) /
      m_container.height()) * 2 + 1;
    m_raycaster.setFromCamera(m_rayMouse, m_camera);
    var intersects = m_raycaster.intersectObjects(windObjs);
    if (intersects.length > 0) {
      if (m_INTERSECTED == null) {
...
function initRayCaster() {
  ...
  m_renderer.domElement.addEventListener('mousemove', onDocumentMouseMoveRay,
    false);
  m_renderer.domElement.addEventListener('touchstart', onDocumentTouchStartRay,
    false);
...
function enableParorama() {
  m_renderer.domElement.addEventListener('mousemove', onDocumentMouseMoveParo,
    false);
  m_renderer.domElement.addEventListener('mousedown', onDocumentMouseDownParo,
    false);
  m_renderer.domElement.addEventListener('mouseup', onDocumentMouseUpParo,
    false);
  m_renderer.domElement.addEventListener('mousewheel', onDocumentMouseWheelParo,
    false);
  m_renderer.domElement.addEventListener('DOMMouseScroll',
    onDocumentMouseWheelParo, false);
  m_renderer.domElement.addEventListener('touchstart', onDocumentTouchStartParo,
    false);
  m_renderer.domElement.addEventListener('touchmove', onDocumentTouchMoveParo,
    false);
}
...
function onDocumentTouchMoveParo(event) {
  if (event.touches.length == 1) {
    event.preventDefault();
    m_lon = (m_onPointerDownPointerX - event.touches[0].pageX) * 0.1 +
      m_onPointerDownLon;
    m_lat = (event.touches[0].pageY - m_onPointerDownPointerY) * 0.1 +
      m_onPointerDownLat;
  }
}
/*
...
function onDocumentMouseMoveParo(event) {
  if (m_isUserInteracting === true) {
    m_lon = (m_onPointerDownPointerX - event.clientX) * 0.1 +
      m_onPointerDownLon;
    m_lat = (event.clientY - m_onPointerDownPointerY) * 0.1 +
      m_onPointerDownLat;
  }
}
...
function init3D() {
  ...
  m_camera = new THREE.PerspectiveCamera(75, m_main_view_width /
    m_main_view_height, 1, 1100);
